...
export function SplitScreenView({ original, result, mode, ageGroup = 'teen' }: SplitScreenViewProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {viewMode === 'split' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div
              className={`p-5 rounded-xl overflow-y-auto min-h-80 max-h-[480px] text-sm content-area ${mode === 'dyslexia' ? 'dyslexia-mode' : mode === 'adhd' ? 'adhd-mode' : 'standard-mode'}`}
              style={{
      ...
      {viewMode === 'adapted' && (
        <div
          className={`p-6 rounded-xl text-sm content-area ${mode === 'dyslexia' ? 'dyslexia-mode' : mode === 'adhd' ? 'adhd-mode' : 'standard-mode'}`}
          style={{
            background: mode === 'dyslexia' ? '#fffbf0' : 'rgba(124,91,249,0.06)',
            border: `1px solid ${modeInfo.color}33`,
            color: mode === 'dyslexia' ? '#2d2d2d' : '#c0c0e0',
